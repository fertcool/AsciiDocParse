...
* text lines (text_line) - the paragraph element are made up of it
* link (links)
...
* style: List of styles of the object
...
This option can hide the url of a link ,hyperlink, media src(image, audio, video) or show it. The default is 'hide_links'
...
>>> for docelem in parser.sources(['cpp']):
...     print(docelem.data)
...     print(docelem.style)
'std::cout << "Hello, World!";'
['cpp']
>>> for docelem in parser.sources([], ['Python']):
...     print(docelem.data)
